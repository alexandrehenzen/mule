...
public final class ConfigurationProviderToolingAdapter extends StaticConfigurationProvider
  ...
  public Set<Value> getConnectionValues(String parameterName) throws ValueResolvingException {
    ...
    ConnectionProvider<?> connectionProvider = configuration.getConnectionProvider()
        .orElseThrow(() -> new ValueResolvingException("Unable to obtain the Connection Provider Instance", CONNECTION_FAILURE));

    ConnectionProvider unwrap = unwrap(connectionProvider);
    ConnectionProviderModel connectionProviderModel =
        getConnectionProviderModel(unwrap, getAllConnectionProviders(getExtensionModel(), getConfigurationModel()))
   ...
   * @param allConnectionProviders list of
   * @return an {@link Optional} value of the {@link ConnectionProviderModel}
   */
  private Optional<ConnectionProviderModel> getConnectionProviderModel(ConnectionProvider connectionProvider,
                                                                       List<ConnectionProviderModel> allConnectionProviders) {
    Class<? extends ConnectionProvider> aClass = connectionProvider.getClass();
    ...
    for (ConnectionProviderModel providerModel : allConnectionProviders) {
      ...
      if (modelProperty.isPresent()) {
        ...
        if (property.getType().equals(aClass)) {
