...
public abstract class ExtensionComponent<T extends ComponentModel> extends AbstractAnnotatedObject
  ...
  public Set<Value> getValues(String parameterName) throws ValueResolvingException {
    try {
      return runWithOptionsContext(context -> withContextClassLoader(getClassLoader(this.extensionModel),
                                                                     () -> valuesProviderMediator
                                                                         .getValues(parameterName, getParameterValueResolver(),
  ...
  private <R> R runWithOptionsContext(Function<ExtensionResolvingContext, R> metadataContextFunction)
