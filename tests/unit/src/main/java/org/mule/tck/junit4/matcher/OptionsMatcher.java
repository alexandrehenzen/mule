 ...
 * @since 4.0
 */
public final class OptionsMatcher extends TypeSafeMatcher<Value> {
  ...
  private OptionsMatcher(String id) {
    this.id = id;
    descriptionBuilder.append(String.format("a MetadataKey with id: '%s'", id));
  }

  public static OptionsMatcher optionWithId(String id) {
    return new OptionsMatcher(id);
  }
  ...
  @Override
  protected boolean matchesSafely(Value metadataKey) {
    try {
  ...
  public OptionsMatcher withDisplayName(String displayName) {
   ...
   * @return the contributed {@link MetadataKeyMatcher}
   */
  public OptionsMatcher withPartName(String partName) {
